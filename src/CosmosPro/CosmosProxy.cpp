...
Control^ CWebRTProxy::GetClient(Form^ form) {
	...
	for each (Control ^ ctrl in form->Controls)
	{
		...
		else if (ctrl->Dock == DockStyle::None) {
			ClientCtrl = ctrl;
		}
	}
	...
	if (_pObj->GetType()->IsSubclassOf(Control::typeid))
	{
		...
		if (pObj != nullptr && pHostNode)
		{
			...
			if (pObj->GetType()->IsSubclassOf(Form::typeid))
			{
				...
				::SetWindowLongPtr(hWnd, GWL_STYLE, ::GetWindowLongPtr(hWnd, GWL_STYLE) & ~(WS_SIZEBOX | WS_POPUP | WS_BORDER | WS_OVERLAPPED | WS_MAXIMIZEBOX | WS_MINIMIZEBOX | WS_THICKFRAME | WS_CAPTION) | WS_CHILD);// | WS_VISIBLE);
...
void CWebRTProxy::CosmosAction(BSTR bstrXml, void* pvoid)
{
	...
	if (strXml != _T(""))
	{
		if (strXml.CompareNoCase(_T("InitAppByLoader")) == 0) {
		...
		if (strXml.CompareNoCase(_T("GetSignData")) == 0) {
			String^ strWhiteList = Forms::Application::ExecutablePath->ToLower();
			strWhiteList = strWhiteList->Replace(L".exe", L"whitelist.dll");
			if (System::IO::File::Exists(strWhiteList)) {
				String^ resourceName = L"whitelist.xml";
				Assembly^ a = nullptr;
				String^ strName = L"";
				try
				{
					a = Assembly::LoadFile(strWhiteList);
				}
				...
				finally
				{
					if (sm != nullptr)
					{
						...
						delete sm;
						CString _strWhiteListXml = strName;
						theApp.m_pWebRT->put_AppKeyValue(CComBSTR("whiltlist"), CComVariant(_strWhiteListXml));
					}
				}
			}
			return;
...
bool CSpaceTelescope::EclipseAppInit()
{
	Universe::WebRT::GetWebRT()->Fire_OnEclipseAppInit();
	return false;
}
