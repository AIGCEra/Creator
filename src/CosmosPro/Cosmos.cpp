...
namespace Universe
{
	...
	Xobj^ WebRT::BindWndNucleusToDOM(Control^ ctrl, String^ key, String^ strXobjXml)
	{
		if (ctrl != nullptr)
		{
			if (ctrl->Dock != DockStyle::Fill && ctrl->Dock != DockStyle::None)
			...
			if (nPointer)
			{
				...
				if (theApp.m_pWebRT == nullptr)
				{
					...
					if (hModule) {
						...
						GetWebRT _pWebRTFunction = nullptr;
	...
	void WebRT::InitEclipse()
	{
		if (theApp.m_pWebRT && !g_pSpaceTelescopeImpl->m_bIsEclipseInit)
			theApp.m_pWebRT->InitEclipseApp();
	}
		...
		if (theApp.m_pWebRT == nullptr)
		{
			...
			if (hModule == nullptr) {
				...
				if (SHGetFolderPath(NULL, CSIDL_PROGRAM_FILES, NULL, 0, m_szBuffer) ==
					...
					m_strProgramFilePath += _T("\\Tangram\\Creator\\universe.dll");
	...
	bool WebRT::WebRuntimeInit()
	{
		...
		if (g_pSpaceTelescopeImpl->m_hWebRTProxyModel) {
			typedef bool(__stdcall* _IsBindWinApp)();
			_IsBindWinApp _pIsBindWinAppFunction;
			_pIsBindWinAppFunction = (_IsBindWinApp)GetProcAddress(g_pSpaceTelescopeImpl->m_hWebRTProxyModel, "IsBindWinApp");
			if (_pIsBindWinAppFunction != NULL) {
				if (_pIsBindWinAppFunction())
					return true;
			}
	...
	WinNucleus^ WebRT::ActiveEclipseTopFrame::get()
	{
		Object^ pRetObject = nullptr;
		if (theApp.m_pWebRT)
		{
			...
			theApp.m_pWebRT->get_Extender(&pExtender);
			if (pExtender)
			{
				...
				{
					INucleus* pGalaxy = nullptr;
					pTopWnd->get_WinNucleus(&pGalaxy);
					if (pGalaxy)
	...
	WorkBenchWindow^ WebRT::ActiveWorkBenchWindow::get()
	{
		Object^ pRetObject = nullptr;
		if (theApp.m_pWebRT)
		{
	...
	void WebRT::Fire_OnEclipseAppInit()
	{
		OnEclipseAppInit();
	}
	...
	WorkBenchWindow^ WebRT::RemoteActiveWorkBenchWindow(String^ appID)
	{
		CComBSTR _strID = marshal_as<CComBSTR>(appID);
		IWebRT* pWebRT = nullptr;
		theApp.m_pWebRT->get_RemoteCosmos(_strID, &pWebRT);
		if (pWebRT)
		{
			IWorkBenchWindow* pTopWnd = nullptr;
			IWebRTExtender* pExtender = nullptr;
