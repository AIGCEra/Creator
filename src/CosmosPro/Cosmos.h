...
namespace Universe
{
	...
	public:
		...
		static Xobj^ BindWndNucleusToDOM(Control^ ctrl, String^ key, String^ strXobjXml);
		...
		static property WinNucleus^ ActiveEclipseTopFrame
		{
			WinNucleus^ get();
		}
	...
	public:
		...
		Xobj^ Observe(String^ strNucleusName, String^ strKey, String^ strXml)
		{
			if (m_pCtrl)
			{
				IXobj* pXobj = nullptr;
				m_pCtrl->Observe(marshal_as<CComBSTR>(strNucleusName), marshal_as<CComBSTR>(strKey), marshal_as<CComBSTR>(strXml), &pXobj);
				if (pXobj)
				{
					return theAppProxy._createObject<IXobj, Xobj>(pXobj);
				}
