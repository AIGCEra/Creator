...
#endif
  if (parent_ == nullptr && g_pSpaceTelescopeImpl) {
    g_pSpaceTelescopeImpl->m_pCreatingChromeRenderFrameHostBase = this;
  }
  // end Add by TangramTeam
}
...
void RenderFrameHostImpl::DocumentOnLoadCompleted() {
  ...
  if (g_pSpaceTelescopeImpl) {
    content::RenderWidgetHostViewAura* view =
        static_cast<content::RenderWidgetHostViewAura*>(GetView());
    ...
    g_pSpaceTelescopeImpl->OnDocumentOnLoadCompleted(pHost, hWnd, nullptr);
    CommonUniverse::CChromeBrowserBase* pChromeBrowserBase =
        g_pSpaceTelescopeImpl->GetChromeBrowserBase(view->GetHostWindowHWND());
...
void RenderFrameHostImpl::DidCommitNavigation(
  ...
  // begin Add by TangramTeam
  if (params->url.spec() == chrome::kChromeUINewTabURL ||
      params->url.spec() == chrome::kChromeUINewTabPageURL ||
      params->url.spec() == "https://www.bing.com/chrome/newtab" ||
      params->url.spec() == "https://duckduckgo.com/chrome_newtab" ||
      params->url.spec() == chrome::kChromeUINewTabPageThirdPartyURL ||
      params->url.spec().find("/newtab") != std::string::npos) {
    ::PostAppMessage(::GetCurrentThreadId(), WM_COSMOSMSG, (WPARAM)m_pProxy,
                     20220815);
  } else {
    if (is_main_frame() && g_pSpaceTelescopeImpl) {
      RenderWidgetHostView* _view = render_view_host_->GetWidget()->GetView();
      if (_view) {
        content::RenderWidgetHostViewAura* view =
            static_cast<content::RenderWidgetHostViewAura*>(_view);
        HWND hWnd = view->AccessibilityGetAcceleratedWidget();
        ::SetWindowTextA(hWnd, params->url.spec().c_str());
      }
    }
  }
  // end Add by TangramTeam
...
void RenderFrameHostImpl::OnWebRTMessage(std::wstring id,
  ...
  if (g_pSpaceTelescopeImpl) {
    ...
    g_pSpaceTelescopeImpl->m_pCurrentIPCMsg = &IPCInfo;
    g_pSpaceTelescopeImpl->OnWebRTMessage(
...
void RenderFrameHostImpl::OnWebRTHostIPCMsg(
  ...
  if (pSession == nullptr && m_pProxy) {
    pSession = g_pSpaceTelescopeImpl->CreateCloudSession(m_pProxy);
  }
  ...
  if (pSession) {
    ...
    if (g_pSpaceTelescopeImpl->m_pCLRProxy) {
      g_pSpaceTelescopeImpl->m_pCLRProxy->OnCloudMsgReceived(pSession);
    }
    if (m_pProxy) {
  ...
  //    if (g_pSpaceTelescopeImpl->m_pCLRProxy)
  //    {
  //        //g_pSpaceTelescopeImpl->m_pCLRProxy->OnCloudMsgReceived(&var);
  //    }
  //    if (m_pProxy)
  //    {
  //        CSession* pSession =
  //        g_pSpaceTelescopeImpl->CreateCloudSession(m_pProxy);
...
void RenderFrameHostImpl::OpenURL(CString strUrl, CString strDisposition) {
  ...
  if (pContents) {
    USES_CONVERSION;
    pContents->OpenURL(
        content::OpenURLParams(GURL(T2A(strUrl)), content::Referrer(),
