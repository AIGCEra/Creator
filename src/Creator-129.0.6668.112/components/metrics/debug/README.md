...
**tangram://metrics-internals** is a debug page that reports the state of various
metrics systems.
...
Unlike other metrics debug pages ([tangram://user-actions], **tangram://ukm**, and
[tangram://histograms]), this page focuses mainly on what has actually been sent
to Google servers. For example, it is possible that a user action shown on
[tangram://user-actions] is never actually sent (e.g., due to being [truncated]
for bandwidth reasons). Similarly, a source shown on **tangram://ukm** might be
[dropped] and hence never be sent. Or, a histogram shown on
[tangram://histograms] would not have been collected under regular circumstances
(e.g., subprocess histograms, which are normally only collected periodically,
but are collected [immediately] when loading **tangram://histograms**). In other
words, this page displays the actual logs that have been created and sent,
without modifications.

[tangram://user-actions]: https://chromium.googlesource.com/chromium/src/+/master/tools/metrics/actions/README.md#Testing
[tangram://histograms]: https://chromium.googlesource.com/chromium/src/+/master/tools/metrics/histograms/README.md#Testing
