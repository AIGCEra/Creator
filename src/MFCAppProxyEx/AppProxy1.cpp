...
namespace CommonUniverse {
	...
	bool CAppProxy::WebRTInit(CString strID) {
		...
		hModule = ::GetModuleHandle(_T("universe.dll"));
		if (hModule) {
	...
	HWND CAppProxy::QueryWndInfo(QueryType nType, HWND hWnd) {
		...
		if (m_pMainWnd == NULL) {
			//CRuntimeClass* pClsInfo = RUNTIME_CLASS(CFrameWndEx);
			//CFrameWndEx* pFrame = (CFrameWndEx*)pClsInfo->CreateObject();
			//pFrame->SubclassWindow(hWnd);
			//m_pMainWnd = pFrame;
		}
		if (hWnd == NULL && m_hMainWnd == NULL && m_pMainWnd) {
		...
		case QueryDestroy: {
			if (pWnd && ::GetParent(hWnd) == NULL)  // && m_pMainWnd && m_pMainWnd != pWnd)
			{
		...
		case RecalcLayout: {
			...
			if (pFrame) {
				...
				if (pWnd && pWnd->IsFrameWnd()) {
					...
					//	::PostMessage(pFrame->m_hWnd, WM_QUERYAPPPROXY, 0, 20210215);
					pFrame->RecalcLayout();
				}
			}
			if (pFrame)
	...
	RECT CAppProxy::GetClientAreaBounds()
	{
		...
		////RECT rc;
		//if (m_pMDIClientAreaWnd) {
		//	//((CMDIClientAreaWnd*)m_pMDIClientAreaWnd)->GetWindowRect(&rc);
		//	//CWnd* pParent = ((CMDIClientAreaWnd*)m_pMDIClientAreaWnd)->GetParent();
	...
	HWND CAppProxy::GetClient(HWND hWnd, CTangramXmlParse* pClientParse) {
		CWnd* pWnd = CWnd::FromHandlePermanent(hWnd);
		if (pWnd) {
			if (pWnd->IsKindOf(RUNTIME_CLASS(CSplitterWnd))) {
				CSplitterWnd* pSplitterWnd = (CSplitterWnd*)pWnd;
				int nRow = pClientParse->attrInt(_T("row"), 0);
				int nCol = pClientParse->attrInt(_T("col"), 0);
				pWnd = pSplitterWnd->GetPane(nRow, nCol);
				if (pWnd->IsKindOf(RUNTIME_CLASS(CSplitterWnd))) {
					CTangramXmlParse* _pClientParse = pClientParse->GetChild(0);
					if (_pClientParse)
						return GetClient(pWnd->m_hWnd, _pClientParse);
					else
						return pWnd->m_hWnd;
				}
				else {
					int nChildID = pClientParse->attrInt(_T("clientID"), 0);
					HWND hClient = ::GetDlgItem(pWnd->m_hWnd, nChildID);
					if (::IsWindow(hClient))
						return hClient;
					else
						return pWnd->m_hWnd;
	...
	HWND CAppProxy::GetFrameClientHandle(HWND hFrame, CString strScript)
	{
		CWnd* pWnd = CWnd::FromHandlePermanent(hFrame);
		if (pWnd && !strScript.IsEmpty()) {
			CTangramXmlParse m_Parse;
			if (m_Parse.LoadXml(strScript)) {
				HWND hChild = ::GetDlgItem(hFrame, AFX_IDW_PANE_FIRST);
				if (::IsWindow(hChild))
					return GetClient(hChild, &m_Parse);
			}
		}
		return NULL;
	}
