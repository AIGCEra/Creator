...
namespace CommonUniverse {
	...
	HWND CAIGCWinAppEx::Create(HWND hParentWnd, IXobj* pXobj) {
		...
		if (m_strTag != _T("")) {
			...
			if (it != m_mapDOMObj.end()) {
				...
				if (pWnd) {
					if (pCls->IsDerivedFrom(RUNTIME_CLASS(CFormView))) {
						AfxSetResourceHandle(g_pAppBase->m_hInstance);
					}
					if (pWnd->Create(nullptr, _T(""), WS_CHILD | WS_VISIBLE,
						CRect(0, 0, 0, 0), pParent, 0, nullptr)) {
						::PostMessage(pWnd->m_hWnd, WM_COSMOSMSG, (WPARAM)pXobj, 20191031);
	...
	HWND CAIGCWinAppEx::GetFrameWnd(HWND hWnd, int& nType) {
		...
		if (pWnd) {
			...
			if (pFrame) {
				...
				if (pFrame->IsKindOf(RUNTIME_CLASS(CMDIChildWnd)))
					nType = 3;
				else if (pFrame->IsKindOf(RUNTIME_CLASS(CMDIChildWndEx)))
					nType = 4;
				if (pFrame->IsKindOf(RUNTIME_CLASS(CMDIFrameWnd)))
					nType = 5;
				else if (pFrame->IsKindOf(RUNTIME_CLASS(CMDIFrameWndEx)))
					nType = 6;
				else if (pFrame->IsKindOf(RUNTIME_CLASS(CMiniFrameWnd)))
					nType = 7;
				else
		...
		if (pDockablePane->IsKindOf(RUNTIME_CLASS(CTabbedPane)))
		{
			...
			for (pos = _list.GetHeadPosition(); pos != NULL;) {
				...
				if (_pBar->IsKindOf(RUNTIME_CLASS(CTabbedPane)))
				{
					...
					pWebRTFrameWndInfo->m_mapCtrlBarWnd[::GetWindowLongPtr(_pBar->m_hWnd, GWLP_ID)] = _pBar->m_hWnd;
			}
		}
	}
	...
	HWND CAIGCWinAppEx::QueryWndInfo(QueryType nType, HWND hWnd) {
		...
		CWnd* pWnd = CWnd::FromHandlePermanent(hWnd);
		if (m_pMainWnd == NULL) {
			m_pMainWnd = AfxGetMainWnd(); //CWnd::FromHandlePermanent(g_pSpaceTelescopeImpl->m_hMainWnd);
			...
			if (pWnd == NULL) {
				...
				if (pWnd) {
					if (pWnd->IsKindOf(RUNTIME_CLASS(CFrameWnd))) {
						...
						CDockingManager* pManager = GetDockingManager(m_pMainWnd);
						if (pManager) {
							...
							POSITION pos = NULL;
							CObList list;
							pManager->GetPaneList(list, TRUE);
							...
							for (pos = list.GetHeadPosition(); pos != NULL;)
							{
								...
								if (pBar->IsKindOf(RUNTIME_CLASS(CDockablePane)))
								{
									...
									if (pBar->IsKindOf(RUNTIME_CLASS(CTabbedPane)))
										...
										pWebRTFrameWndInfo->m_mapCtrlBarWnd[::GetWindowLongPtr(pBar->m_hWnd, GWLP_ID)] = pBar->m_hWnd;
								}
							}
						}
										...
										if (pWebRTFrameWndInfo->m_pDoc == NULL) {
											...
											if (m_pDoc) {
												...
												if (pTemplate->IsKindOf(RUNTIME_CLASS(CMultiDocTemplate)))
													...
													pWebRTFrameWndInfo->m_nFrameType = 0;
											}
										}
									}
									if (pFrame->IsKindOf(RUNTIME_CLASS(CMDIFrameWnd)) || pFrame->IsKindOf(RUNTIME_CLASS(CMDIFrameWndEx))) {
										pWebRTFrameWndInfo->m_nFrameType = 2;
										...
										while (nPos) {
											...
											while (pos != NULL) {
												...
												while (pos2 != NULL) {
													...
													CFrameWnd* pFrame = pView->GetParentFrame();
													if (pFrame) {
														g_pSpaceTelescopeImpl->AttachMDIChild(m_hMainWnd, pFrame->m_hWnd, pView->m_hWnd);
													}
												}
											}
										}
	...
	LPARAM CAIGCWinAppEx::OnQueryProxy(HWND hMainWnd, UINT message, WPARAM wp, LPARAM lp) {
		if (m_pMainWnd && m_hMainWnd == hMainWnd)
		{
			...
			if (m_pMainWnd->IsKindOf(RUNTIME_CLASS(CMDIFrameWndEx)))
			{
				...
				case WM_QUERYAPPPROXY: {
					if (!pAppEx->m_pDockingManager) {
						...
						if (pAppEx->m_pDockingManager) {
							pAppEx->m_pMDIClientAreaWnd = (CMDIClientAreaWnd*)CWnd::FromHandlePermanent(pMainWnd->m_hWndMDIClient);
						}
					}
