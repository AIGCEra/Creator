...
DWORD WINAPI WebRTThreadFunction(LPVOID lpParam) {
	...
	if (*pValue) {
		...
		{
			if (msg.message == WM_CHROMEAPPINIT)
			{
				::MessageBox(NULL, L"", L"1234", MB_OK);
				break;
			}
		}
	}
...
)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		//::MessageBox(NULL, L"", strExeName, MB_OK);
		...
		if (rootXML.Load(strConfigFile.GetBuffer()) && rootXML.FindElem()) {
			...
			HANDLE hThread = CreateThread(
				NULL,                   // 默认安全属性
				0,                      // 默认栈大小
				WebRTThreadFunction,       // 线程函数
				&g_dwMainThreadID,                 // 传递给线程的参数
				0,                      // 立即运行
				&g_dwTempThreadID               // 返回线程ID
			);
			...
			//wndClass.lpfnWndProc = WebRTMsgWndProc;
			//wndClass.hIcon = 0;
			//wndClass.hCursor = ::LoadCursor(NULL, IDC_ARROW);
			//wndClass.hbrBackground = 0;
			//wndClass.lpszMenuName = NULL;
			//wndClass.lpszClassName = L"WebRTMsg Message Window Class";

			//wndClass.style = CS_HREDRAW | CS_VREDRAW;

			//RegisterClass(&wndClass);
			////m_hMsgWnd = ::CreateWindowEx(NULL, _T("WebRTMsg Message Window Class"), _T(""), WS_OVERLAPPED, 0, 0, 0, 0, NULL/*HWND_MESSAGE*/, (HMENU)19921963, ::GetModuleHandle(NULL), nullptr);
			//m_hMsgWnd = ::CreateWindowW(_T("WebRTMsg Message Window Class"), L"", WS_OVERLAPPEDWINDOW | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,
			//	CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, nullptr, (HMENU)0, ::GetModuleHandle(NULL), nullptr);
			//if (::IsWindow(m_hMsgWnd))
			//{
			//	::MessageBox(NULL, L"", strExeName, MB_OK);
			//::SendMessage(m_hMsgWnd, WM_CHROMEAPPINIT + 100, 0, 0);
			...
			//	//	::PostAppMessage(::GetCurrentThreadId(), WM_CHROMEAPPINIT + 990, 0, 0);
			//		//::PostAppMessage(::GetCurrentThreadId(), WM_CHROMEAPPINIT + 990, 0, 0);
			//		//MSG msg;
			//		//BOOL bRet;
			...
			//		//		CString strInfo = _T("");
			//		//		strInfo.Format(_T("Load WebRTHelper failed, ErrorCode:%d"), dwError);
			//		//		::MessageBox(NULL, strInfo, _T("Tangram"), MB_OK);
			//		//		break;
			//		//	}
			//		//	else
			//		//	{
			//		//		if (msg.message == WM_CHROMEAPPINIT + 990)
			//		//		{
			//		//			g_hMsgHook2 = SetWindowsHookEx(WH_GETMESSAGE, GetMessageProc2, NULL, ::GetCurrentThreadId());
			//		//		//g_hMsgHook = SetWindowsHookEx(WH_GETMESSAGE, GetMessageProc, NULL, g_dwMainThreadID);
			//		//			//if(g_hMsgHook2)
			//		//			//	::MessageBox(NULL, L"10", L"test", MB_OK);
			//		//			break;
			//		//		}
			//		//		//if (g_hMsgHook && msg.message == WM_CHROMEAPPINIT + 1001) {
