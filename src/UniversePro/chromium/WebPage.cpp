...
namespace Browser {
	...
	{
		if (m_pChromeRenderFrameHost)//::(GetHostBrowserWnd(), GWL_STYLE) & WS_CHILD)
		{
			...
			//m_pChromeRenderFrameHost->SetFocus(true); 
		...
		case 20190331:
		{
			if (m_pChromeRenderFrameHost)
			{
				...
				else
				{
					...
					m_pChromeRenderFrameHost->SetFocus(false);
				}
			}
		}
		break;
	...
	void CWebView::CreateTabGroup(CString strHTML)
	{
		...
		if (m_Parse.LoadXml(strHTML))
		{
			...
			if (nCount > 0)
			{
				for (int i = 0; i < nCount; i++)
				{
					CString strUrl = m_Parse.GetChild(i)->attr(_T("url"), _T("tangram://newtab"));
					int nPos2 = strUrl.Find(_T(":"));
					if (nPos2 != -1)
					{
						CString strURLHeader = strUrl.Left(nPos2);
