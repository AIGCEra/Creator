...
namespace Browser {
	...
	void CBrowser::BrowserLayoutImpl()
	{
		bool bHasDrawWnd = ::IsWindow(m_hDrawWnd);
		HWND _hWebPage = m_pVisibleWebView->m_hWnd;
		HWND hExtendWnd = m_pVisibleWebView->m_hExtendWnd;
		RECT rcExtendWnd;
		if (bHasDrawWnd) {
			...
			HWND h = bHasDrawWnd ? m_hWnd : hExtendWnd;
			if (bHasDrawWnd)
			{
			...
			//浏览器页面扩展窗口区域：
			HRGN hWebPage = NULL;
			HRGN hWebExtendWndRgn = ::CreateRectRgn(rcExtendWnd.left, 0, rcExtendWnd.left + m_nWidth, rcExtendWnd.top + m_nHeigh + m_nTop);
			//浏览器页面窗口区域：
			if (m_bSZMode == false)
			{
				RECT rcWebPage;
			...
			if (bHasDrawWnd)
			{
				...
				::ScreenToClient(h, (LPPOINT)&rcBrowser);
				::ScreenToClient(h, ((LPPOINT)&rcBrowser) + 1);
				//浏览器窗口区域：
				HRGN hWebExtendWndRgn2 = ::CreateRectRgn(rcExtendWnd.left, 0, rcExtendWnd.left + m_nWidth, m_nTop);
				...
				::CombineRgn(hGPUWndRgn, hGPUWndRgn, hWebExtendWndRgn2, RGN_OR);
				::CombineRgn(hGPUWndRgn, hGPUWndRgn, hWebPage, RGN_OR);
				::SetWindowRgn(hExtendWnd, hWebExtendWndRgn, true);
				::SetWindowRgn(m_hDrawWnd, hGPUWndRgn, true);
...
#endif
	void CBrowser::OnEnterSizeMove() {
		m_bSZMode = true;
		g_pSpaceTelescope->m_mapSizingBrowser[m_hWnd] = this;
	}

	void CBrowser::OnExitSizeMove() {
		m_bSZMode = false;
	...
	LRESULT CBrowser::OnActivate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	...
	LRESULT CBrowser::OnMouseActivate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
		if (m_pVisibleWebView)
		{
	...
	LRESULT CBrowser::OnWebRTMsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
		LRESULT lRes = DefWindowProc(uMsg, wParam, lParam);
		if (m_bDestroy)
			return lRes;
	...
	LRESULT CBrowser::OnDestroy(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		m_bDestroy = true;
		...
		//else if (g_pSpaceTelescope->m_mapBrowserWnd.size() == 1)
		//{
		//	if (g_pSpaceTelescope->m_hMainWnd) {
		//		HWND hBrowser = g_pSpaceTelescope->m_mapBrowserWnd.begin()->first;
		//		HWND hTop = ::GetAncestor(hBrowser, GA_ROOT);
		//		if (hTop != g_pSpaceTelescope->m_hMainWnd)
	...
	LRESULT CBrowser::OnChromeTabChange(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
		LRESULT lRes = DefWindowProc(uMsg, wParam, lParam);
	...
	LRESULT CBrowser::OnWindowPosChanging(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	...
	LRESULT CBrowser::OnExitSZ(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
		m_bSZMode = false;
		for (auto& it : g_pSpaceTelescope->m_mapSizingBrowser)
		{
			if (::IsWindow(it.first))
	...
	LRESULT CBrowser::OnEnterSZ(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
		m_bSZMode = true;
		g_pSpaceTelescope->m_mapSizingBrowser[m_hWnd] = this;
		LRESULT lRes = DefWindowProc(uMsg, wParam, lParam);
		return lRes;
	...
	LRESULT CBrowser::OnBrowserLayout(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		LRESULT lRes = DefWindowProc(uMsg, wParam, lParam);
