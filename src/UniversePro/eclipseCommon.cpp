...
/* Global Variables */
_TCHAR* osArg        = (_TCHAR*)_T_ECLIPSE(DEFAULT_OS);
#ifdef MACOSX
	...
	#endif
#else
_TCHAR* osArchArg    = (_TCHAR*)_T_ECLIPSE(DEFAULT_OS_ARCH);
#endif
_TCHAR* wsArg        = (_TCHAR*)_T_ECLIPSE(DEFAULT_WS);	/* the SWT supported GUI to be used */
...
static int compareVersions(const _TCHAR* str1, const _TCHAR* str2) {
	...
	if(result == 0) {
		_TCHAR * q1 = v1->qualifier ? v1->qualifier : (_TCHAR*)_T_ECLIPSE("");
		_TCHAR * q2 = v2->qualifier ? v2->qualifier : (_TCHAR*)_T_ECLIPSE("");
